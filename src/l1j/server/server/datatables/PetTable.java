...
public class PetTable {
	...
	public void storeNewPet(L1NpcInstance pet, int objid, int itemobjid) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"INSERT INTO pets SET item_obj_id=?,objid=?,npcid=?,name=?,lvl=?,hp=?,mp=?,exp=?,lawful=?,food=?");
	...
	public void storePet(L1Pet pet) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement(
					"UPDATE pets SET objid=?,npcid=?,name=?,lvl=?,hp=?,mp=?,exp=?,lawful=?,weapon=?,armor=?,food=? WHERE item_obj_id=?");
	 ...
	 *
	 * @param nameCaseInsensitive Check your pet's name. Case differences are
	 *                            ignored.
	...
	public static boolean isNameExists(String nameCaseInsensitive) {
		...
		try {
			...
			rs = pstm.executeQuery();
			if (!rs.next() || PetTypeTable.getInstance().isNameDefault(nameLower)) { // The
																		// default
																		// name
																		// is
																		// not
