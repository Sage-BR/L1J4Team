...
public class C_Mail extends ClientBasePacket {
	...
	public C_Mail(byte abyte0[], Client client) throws Exception {
		...
		if (type == 0x00 || type == 0x01 || type == 0x02) { // Open
			...
			byte[] text = readByte();
			L1PcInstance receiver = CharacterTable.getInstance().restoreCharacter(receiverName);
			if (receiver == null) {
			...
			if (clan != null) {
				for (String name : clan.getAllMembers()) {
					...
					int mailId = MailTable.getInstance().writeMail(TYPE_CLAN_MAIL, name, pc, text, clanPc.getId());
