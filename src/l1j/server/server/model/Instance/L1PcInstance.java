...
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_AMULET;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_ARMOR;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_BELT;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_BOOTS;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_CLOAK;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_EARRING;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_GLOVE;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_HELM;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_RING1;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_RING2;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_RING3;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_RING4;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_RUNE1;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_RUNE2;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_RUNE3;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_RUNE4;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_RUNE5;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_SHIELD;
import static l1j.server.server.serverpackets.S_EquipmentWindow.EQUIPMENT_INDEX_T;
...
public class L1PcInstance extends L1Character {
	...
	public void teleWithDelay(int delay, int x, int y, short mapid, int head, boolean ignorePets) {
		...
		_teleDelayFuture = GeneralThreadPool.getInstance().schedule(new TeleDelay(this, x, y, mapid, head, ignorePets),
				delay);
	}
	...
	private List<L1PrivateShopBuyList> _buyList = new ArrayList<>();
	...
	private List<L1PrivateShopSellList> _sellList = new ArrayList<>();
	...
	public L1BookMark getBookMark(int id) {
		for (L1BookMark element : _bookmarks) {
			if (element.getId() == id) {
				return element;
			}
	...
	public void onPerceive(L1PcInstance perceivedFrom) {
		if (isGmInvis() || isGhost() || (isInvisble() && !perceivedFrom.hasSkillEffect(GMSTATUS_FINDINVIS))) {
			return;
		}
