...
public class L1ItemInstance extends L1Object implements Comparable<L1ItemInstance> {
	...
	public int getMr() {
		...
		if (getItemId() == 20056 || getItemId() == 120056 || getItemId() == 220056 || getItemId() == 20049
				|| getItemId() == 20050) {
	...
	public byte[] getStatusBytes() {
		...
		if (itemType2 == ItemType.Etc) {
			...
			if (getEnchantLevel() != 0) {
				os.writeC(2);
				if (isArmor && template.getType() >= 8 && template.getType() <= 12) {
					os.writeC(0);
				} else {
			...
			/*
			 * Taken from l1j-tw accessory enchanting - these might not be the right packets
			 * for US clients and could be causing the mangled item descriptions. if
			 * (getMagicResist() != 0) { os.writeC(15); os.writeH(getMagicResist()); } if
			 * (template.get_addhpr() != 0 || getAddHpRegen() != 0) { os.writeC(37);
			 * os.writeC(template.get_addhpr() + getAddHpRegen()); } if
			 * (template.get_addmpr() != 0 || getAddMpRegen() != 0) { os.writeC(38);
