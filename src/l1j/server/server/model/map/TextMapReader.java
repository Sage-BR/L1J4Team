...
public class TextMapReader extends MapReader {
	...
	public L1Map read(final int id) {
		...
		try {
			map = new L1V1Map(id, this.read(id, xSize, ySize), MapsTable.getInstance().getStartX(id),
					MapsTable.getInstance().getStartY(id), MapsTable.getInstance().isUnderwater(id),
					MapsTable.getInstance().isMarkable(id), MapsTable.getInstance().isTeleportable(id),
					MapsTable.getInstance().isEscapable(id), MapsTable.getInstance().isUseResurrection(id),
					MapsTable.getInstance().isUsePainwand(id), MapsTable.getInstance().isEnabledDeathPenalty(id),
					MapsTable.getInstance().isTakePets(id), MapsTable.getInstance().isRecallPets(id),
					MapsTable.getInstance().isUsableItem(id), MapsTable.getInstance().isUsableSkill(id));
